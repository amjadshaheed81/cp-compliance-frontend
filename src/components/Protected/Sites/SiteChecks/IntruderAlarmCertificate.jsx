...
const IntruderAlarmCertificate = ({
              ...
              if (plantAndEquipmentResponse?.document?.childFolders) {
                ...
                if (miscellaneousFolder) {
                  ...
                  if (miscResponse?.document?.childFolders) {
                    const intruderAlarmFolder = miscResponse.document.childFolders.find(
                        folder => folder.name.trim() === 'Intruder Alarm Service & Maintenance'
                    );
                    ...
                    setFolderIds({
                      ...
                      intruderAlarmService: intruderAlarmFolder?.id || null
                    });

                    return intruderAlarmFolder?.id || null;
                  }
                }
              }
            }
  ...
  useEffect(() => {
    const shouldShowRiskAssessment = [ "param5", "param6", "param7", "param8"].some(
  ...
  }, [formData.param5, formData.param6, formData.param7, formData.param8, currentCheckId, existingAction]);
  ...
  const uploadPdfToServer = async (pdfBlob, fileName) => {
    try {
      ...
      const targetFolderId = folderIds.intruderAlarmService || folderIds.logBooks;
  ...
  const generatePDF = async (uploadToServer = true) => {
    try {
      ...
      setTextField('Panel Details', equipmentDetailsLocation, smallFont);
      ...
      setTextField('Walk Test', formData.param3 === 'Pass' ? 'Yes' : 'No', smallFont);
      setTextField('PIRsCleaned', formData.param4 === 'Pass' ? 'Yes' : 'No', smallFont);
      setTextField('Remote', formData.param5 === 'Pass' ? 'Pass' : 'Fail', smallFont);
      setTextField('System Check', formData.param6 === 'Pass' ? 'Pass' : 'Fail', smallFont);
      setTextField('Audible Check', formData.param7 === 'Pass' ? 'Pass' : 'Fail', smallFont);
      setTextField('Electrical Check', formData.param8 === 'Pass' ? 'Pass' : 'Fail', smallFont);
      ...
      const fileName = `IntruderAlarmServiceCertificate_${formData.selectedAsset?.assetName || 'report'}.pdf`;
