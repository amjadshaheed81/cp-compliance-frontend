  ...
  const searchSiteCheck = () => {
    ...
    if (formData2?.searchField?.length > 0 && filteredSiteChecks2?.length > 0) {
      filteredSiteChecks2.forEach(s => {
        ...
        if (lead.length > 0) {
          s.leadName = lead[0].role + ' - ' + lead[0].name + ' (' + lead[0].email + ')' + (lead.companyName ? " - " + lead.companyName : "");
        }
      })
              ...
              <tbody>
                {!isLoading && filteredSiteChecks?.length === 0 && (
                  ...
                  {!isLoading && currentSiteChecks?.map((action) =>
                  {
                    ...
                    if (lead.length > 0) {
                      leanName = lead[0].role + ' - ' + lead[0].name + ' (' + lead[0].email + ')' + (lead.companyName ? " - " + lead.companyName : "");
                    }
                    return (
                    <tr key={action?.id}>
                  ...
                  <div style={{ margin: "10px" }}>
                    ...
                    <Autocomplete
                      ...
                      onChange={(event, item) => {
                        ...
                        value={managerList.filter(o => String(o.id) === String(formData?.leadUserID)).map((option) => { return { key: option.id, label: option.role + ' - ' + option.name + ' (' + option.email + ')' + (option.companyName ? " - "+option.companyName : "") } })[0]}
