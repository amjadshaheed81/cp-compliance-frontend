...
import { Button, DialogContent, DialogTitle, DialogActions, Dialog, Typography, Box, Grid, Divider, TextField, Autocomplete } from "@mui/material";
import { deleteUser, getSites, getUsers, getSiteAssets, getSiteLayout } from "../../../../store/thunk/site";

const SurveyWaterOutletTemperature = ({ checkId, siteAssets, siteLayout, getUsers, getSiteAssets, siteSelectedForGlobal, getSiteLayout }) => {
  ...
  const getName = (idx) => {
    ...
    console.log('resresres', floor, formData[idx])
    return res;
  }
                ...
                <tr>
                  <td>
                    ...
                    <td style={{ width: '200px' }}>
                      <p style={{ lineHeight: '3px' }}>1st : {formData?.[idx]?.reading1} ({formData?.[idx]?.r1Date?.substring(0, 10)})</p>
                      <p style={{ lineHeight: '3px' }}>2nd : {formData?.[idx]?.reading2} ({formData?.[idx]?.r2Date?.substring(0, 10)})</p>
                      <p style={{ lineHeight: '3px' }}>3rd : {formData?.[idx]?.reading3} ({formData?.[idx]?.r3Date?.substring(0, 10)})</p>
                    </td>
