...
const EnergyMetricsReport = ({ energyData, floorArea, currentYear }) => {
  ...
  const processYearlyData = (data, year) => {
    ...
    metrics.carbonFootprint = metrics.totalElectricityKwh * 0.35 + metrics.totalGasKwh * 0.19;
          ...
          label: (context) => {
            ...
            if (context.parsed.y !== null) {
              ...
              if (context.datasetIndex === 0 && context.dataIndex === 0) {
                ...
                label += context.dataset.label.includes('Electricity') ? ' kWh/m²' : 
                         context.dataset.label.includes('Gas') ? ' kWh/m²' : ' £/m²';
              }
            }
            return label;
          }
  ...
  return (
    <div className="container">
      <div className="row">
        <div className="col-md-6">
          <div className="bg-white p-4 rounded-lg shadow mb-4">
            ...
            <div className="mt-4 text-sm text-gray-600">
              <p>Total Cost: £{currentYearData.totalCost.toLocaleString('en-GB', { 
                minimumFractionDigits: 2,
                maximumFractionDigits: 2 
              })}</p>
              <p>Cost per m²: £{currentYearData.costPerM2.toFixed(2)}</p>
              {floorArea && <p className="text-xs mt-1">Based on {floorArea.toLocaleString()} m² floor area</p>}
            </div>
          </div>
        </div>
        ...
        <div className="col-md-6">
          <div className="bg-white p-4 rounded-lg shadow mb-4">
            ...
            <div className="mt-4 text-sm text-gray-600">
              <p>Total Carbon: {currentYearData.carbonFootprint.toFixed(2)} kg CO2e</p>
              <p>Carbon per m²: {currentYearData.carbonPerM2.toFixed(2)} kg CO2e</p>
              <p className="text-xs mt-1">(Electricity: 0.35 kg/kWh, Gas: 0.19 kg/kWh)</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="row">
        <div className="col-md-6">
          <div className="bg-white p-4 rounded-lg shadow mb-4">
            <h2 className="text-lg font-semibold mb-4">Energy Consumption per m²</h2>
            <Bar data={chartConfigs.consumptionPerM2} options={chartOptions} />
            <div className="mt-4 text-sm text-gray-600">
              <p>Total Electricity: {currentYearData.totalElectricityKwh.toLocaleString('en-GB')} kWh</p>
              <p>Total Gas: {currentYearData.totalGasKwh.toLocaleString('en-GB')} kWh</p>
            ...
            <div className="mt-4 text-sm text-gray-600">
              ...
              {!floorArea && (
                <p className="text-red-500 text-xs mt-1">
                  Warning: Floor area not provided - per m² calculations unavailable
                </p>
              )}
            </div>
          </div>
